...
void AddLineDialog::acc() {
    ...
    if (fileNameDirectionA != "" && fileNameDirectionB != "") {
        ...
        if ((line = network_.getLine(ui->lineName->text().toStdString())) == nullptr) {
            line = new BusLine(ui->lineName->text().toStdString(), ui->firstStop->text().toStdString(),
                               ui->lastStop->text().toStdString());
...
void AddLineDialog::dirA() {
    fileNameDirectionA = QFileDialog::getOpenFileName(this, tr("Select file for direction A"), "/",
                                                      tr("Text Files (*.txt)"));
}

void AddLineDialog::dirB() {
    fileNameDirectionB = QFileDialog::getOpenFileName(this, tr("Select file for direction B"), "/",
                                                      tr("Text Files (*.txt)"));
}
